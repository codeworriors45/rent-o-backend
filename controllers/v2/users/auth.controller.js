...
const register = async(req, res = response) => {
    let rules = {
        ...
        referenceBy: "required|string|min:1|max:50"
        // gender: "required"
    };
    ...
    try {
        ...
        let { id, email, password, firstName, lastName, dob, referenceBy, phoneNumber, description, type, createdAt, updatedAt, deletedAt } = req.body;
        ...
        const user = await db.User.create({
            firstName, lastName, email, password, type, dob, referenceBy, description, phoneNumber
        });
        ...
        // let link
        // if (process.env.BACKEND_LINK) {
        //     link = `${process.env.BACKEND_LINK}/v2/auth/verify?i=${user.id}&h=${hash}&t=${date}`
        // } else {
        //     link = `${req.baseUrl}/verify?i=${user.id}&h=${hash}&t=${date}`
        // }
        let link = `https://${req.headers.host}${req.baseUrl}/verify?i=${user.id}&h=${hash}&t=${date}`
...
const resendEmailVerify = async (req, res, next) => {
    try{
        ...
        if (process.env.BACKEND_LINK) {
            ...
            link = `${req.protocol}://${req.headers.host}/v2/auth/verify?i=${user.id}&h=${hash}&t=${date}`
        }
