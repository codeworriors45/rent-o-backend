...
const propertyController = {
  index: async (req, res) => {
    try {
      const { approve, page, limit, orderby, order } = req.query;
      ...
      const properties = await db.Property.findAll({
        ...
        include: [
          ...
          {
            model: db.SellerPropertyContract,
            attributes: ["signingId", "signatureRequestId", "isSigned"],
          },
          {
          ...
          },
          {
            ...
            attributes: ['id', 'sellerId', 'buyerId', 'sellerSignStatus', 'buyerSignStatus'],
            where: {
              sellerSignStatus: {
                [Op.is]: true
              },
    ...
    try {
      ...
      if (user == null || user.length == 0) {
        return apiResponse.error(
          res,
          httpStatusCode.NOT_FOUND,
          "No user found with specific id."
        );
      }
      ...
      if (listingType == null || listingType.length == 0) {
        return apiResponse.error(
          res,
          httpStatusCode.NOT_FOUND,
          "No listing type found with specific id."
        );
      }
