...
const store = async(req,res) => {
    try {
        ...
        let { name, code, img, sequence, visible } = req.body;
        ...
        let imgValid = imageValidation.base64(img);
        if(!imgValid) {
        ...
        const imageUpload = await cloudinaryHelper.upload(img, 'provinces');
        let description = '';
        console.log(imageUpload);
        if(imageUpload.success){
            description = JSON.stringify(imageUpload.cloudimg)
        }
