...
const store = async(req,res) => {
    try {
        ...
        let rules = {
            ...
            buyerSigningId: 'required|string',
            expireAt: "date",
            closeAt: "date"
        };
        ...
        let { propertyId, includeItem, condition, note, 
            expireAt, closeAt, contractId, offerPrice, buyerSigningId } = req.body;
        /// TO DO : check    
        const prevData = await db.Property.findOne({
            where: {
                id: propertyId,
                soldAt: {
                    [Op.is]: null
        ...
        let offer = await db.Offer.create({
            propertyId, userId, preQualified : user.verified,
            includeItem, condition, note, 
            expireAt, closeAt, contractId,
            offerPrice, buyerSigningId
        });
    ...
    try {
        ...
        const offer = await db.Offer.findAll({
            ...
            include: [
                ...
                {
                    model: db.BuyerSellerContract,
                    attributes: ["signatureRequestId","buyerSignId","sellerSignId", "sellerSignStatus", "buyerSignStatus", "isApproved"]
                }
            ]
        });
...
const update = async(req,res) => {
    try {
        ...
        const { id } = req.params;
        let { preQualified, includeItem, condition, 
              note, expireAt, closeAt, 
              buyerSignId, contractId, offerPrice } = req.body;
        ...
        offer.update({ preQualified, includeItem, condition, 
                       note, expireAt, closeAt,
                       buyerSignId, contractId, offerPrice });
